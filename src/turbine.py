...
class TurbineData:
    ...
    def load_operational_strategy(self):
        ...
        Returns:
            dict: A dictionary containing wind speed (u), axial induction factor (a), rotational speed (w), power (P), and torque (T).
        """
        data = np.genfromtxt(self.operational_strategy_path, delimiter='', skip_header=1)
        u, a, w, P, T = data[:, 0], data[:, 1], data[:, 2], data[:, 3], data[:, 4]
        self.operational_strategy_data = {"u": u, "a": a, "w": w, "P": P, "T": T}
